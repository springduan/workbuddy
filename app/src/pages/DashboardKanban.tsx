...
export function DashboardKanban({
  ...
  }, [scheduleTasks, searchTerm, salesFilter, vehicles]);
  ...
  const allSalesmen = useMemo(() => {
    ...
    scheduleTasks.forEach((task) => {
      task.customers.forEach((c) => {
        if (c.salesman) names.add(c.salesman);
      });
    });
  ...
  const stats = useMemo(() => {
    ...
    tasksForDate.forEach((task) => {
      task.customers.forEach((c) => {
        if (c.salesman) salesmanSet.add(c.salesman);
      });
    });
          ...
          <CardContent className="p-4 flex flex-col sm:flex-row gap-3">
            ...
            {allSalesmen.length > 0 && (
              <select
                ...
                className="h-10 px-3 rounded-lg border border-gray-300 bg-white text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400"
              >
                <option value="all">全部业务员</option>
                {allSalesmen.map((name) => (
            ...
            {salesFilter !== 'all' && (
              <Button
                ...
                className="h-10 px-3 text-gray-500"
              >
                清除筛选
              </Button>
            )}
            ...
            <div className="flex flex-wrap gap-2 pl-1 pt-1">
              {task.customers.map((c) =>
                (c.salesman || c.salesmanPhone || c.company) ? (
                  <div
                    key={`sales-${c.id}`}
                    className="flex items-center gap-2 text-xs bg-blue-50 border border-blue-200 px-2 py-1 rounded-lg"
                  >
                    <Briefcase className="w-3 h-3 text-blue-500 shrink-0" />
                    <span className="font-medium text-blue-800">{c.salesman}</span>
                    {c.salesmanPhone && (
                      <a
                        href={`tel:${c.salesmanPhone}`}
                    ...
                    {c.company && (
                      <>
                        ...
                        <span className="text-blue-600">{c.company}</span>
                      </>
                    )}
                  </div>
                ) : null
    ...
    <div className="p-3 space-y-3">
      ...
      {task.customers.some((c) => c.salesman || c.salesmanPhone || c.company) && (
        <div className="mt-2 pt-2 border-t border-blue-200">
          <div className="text-xs font-bold text-blue-700 mb-1">业务员信息</div>
          {task.customers
            .filter((c) => c.salesman || c.salesmanPhone || c.company)
            .map((customer: Customer) => (
              <div
                key={`tooltip-sales-${customer.id}`}
                className="bg-blue-50 px-2 py-1 rounded flex items-center gap-2 text-xs mb-1"
              >
                <Briefcase className="w-3 h-3 text-blue-500 shrink-0" />
                {customer.salesman && (
                  <span className="font-medium text-blue-800">{customer.salesman}</span>
                )}
                {customer.salesmanPhone && (
                  <a
                    href={`tel:${customer.salesmanPhone}`}
                ...
                {customer.company && (
                  <span className="text-blue-500">· {customer.company}</span>
                )}
              </div>
            ))}
        </div>
